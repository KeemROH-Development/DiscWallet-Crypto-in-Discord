...
client.on("message", message => {
  ...
  let isstarted = db.fetch(`isstarted_${message.author.id}`)
   ...
   if (command.mustStart && !isstarted) {
    return message.reply("Hello, It seems that you have not started yet! To begin please use `d!start`!");
  }
...
const loadEvents = (module.exports.loadCommands = (dir = "./events/") => {
  fs.readdir(dir, (error, files) => {
    ...
    files.forEach(file => {
      ...
      client.on(props.event.name, props.bind(null, client));
    });
  });
});
loadEvents();
